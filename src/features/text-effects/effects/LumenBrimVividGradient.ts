export interface LumenBrimVividGradientConfig {
  width: number;
  height: number;
  text: string;
  fontFamily?: string;
...
export class LumenBrimVividGradientEngine {
  private cfg: Required<LumenBrimVividGradientConfig>;

  constructor(config: LumenBrimVividGradientConfig) {
    // Merge provided configuration with static studio defaults
    const defaults: Required<LumenBrimVividGradientConfig> = {
      width: 800,
      height: 200,
      text: "TEXT",
  ...
  drawFrame(ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, ghostFrames?: ImageData[]): void {
    ...
    const isGlitchEffect = "LumenBrimVividGradient".toLowerCase().includes("glitch") || text === "SYSTEM ERR";
    if (isGlitchEffect) {
      ctx.save();
      ctx.globalAlpha = 0.8;
...
export const LumenBrimVividGradientDefinition = {
  id: "lumen-brim",
  name: "LumenBrim",
  text: "CLYPRA",
  category: "classic",
